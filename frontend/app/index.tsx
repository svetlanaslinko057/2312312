...
export default function Index() {
  ...
  // Welcome gate: unauthenticated visitors landing on `/` for the first
  // time in this session must see /welcome first. Once they continue from
  // there, the session flag is set and this redirect becomes a no-op.
  // Authenticated users are already handled by the redirect above.
  useEffect(() => {
    if (authLoading) return;
    if (token) return;
    if (!hasWelcomeBeenSeenInSession()) {
