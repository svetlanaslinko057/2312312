...
/* ─── colour helpers — bound to design-system semantic tokens ─────────────
   Was bright dashboard alarms (#ef4444 / #f59e0b / #10b981 / #06b6d4).
    ...
    <div className="p-5 max-w-[1600px] mx-auto" data-testid="admin-execution-intelligence">
      ...
      {err && (
        ...
        >
          ...
          <span className="text-[13px]" style={{ color: '#ef4444' }}>{err}</span>
        </Card>
      )}
...
function Header({ conviction, suppressions, decisions24h, onRefresh, lastTick }) {
  ...
  return (
    <Card
      ...
      style={{
        boxShadow: `0 0 0 1px var(--token-border), 0 0 32px ${bs.glow}`,
      }}
    >
          ...
          {conviction.components.map((c, i) => (
            ...
            >
              ...
              <span
                ...
                style={{ color: c.delta > 0 ? '#10b981' : c.delta < 0 ? '#ef4444' : 'var(--token-muted)' }}
              >
                {c.delta > 0 ? '+' : ''}{c.delta}
              </span>
            </div>
...
function LiveFlowPanel({ pipeline, stream, decisions24h, selected, onSelect }) {
  const buckets = [
    { key: 'open',        label: 'Open',        icon: CircuitBoard, color: '#f59e0b' },
    { key: 'evaluating',  label: 'Evaluating',  icon: Workflow,     color: '#06b6d4' },
    { key: 'in_progress', label: 'In progress', icon: Activity,     color: '#06b6d4' },
    { key: 'review',      label: 'Review',      icon: Shield,       color: '#a78bfa' },
    { key: 'completed',   label: 'Completed',   icon: Sparkles,     color: '#10b981' },
    { key: 'failed',      label: 'Failed',      icon: Flame,        color: '#ef4444' },
  ];
  return (
      ...
      <div className="space-y-1.5 max-h-[480px] overflow-auto pr-1" data-testid="cognition-stream">
        ...
        {stream.map((s) => {
          ...
          const dotColor =
            ...
            status.includes('block') || status === 'logged_only' ? '#ef4444' :
  ...
  if (why.status === 'not_found') {
    return (
      <Card className="p-6 h-full">
        <Kicker color="#ef4444">Module not found</Kicker>
          ...
          return (
            <div key={`${ev.phase}-${i}-${ev.at}`} className="relative" data-testid={`timeline-phase-${ev.phase}`}>
              <span
                ...
                style={{
                  background: bs.color,
                  boxShadow: isLast ? `0 0 0 4px ${bs.glow}` : 'none',
...
function UniversesPanel({ universes, selected }) {
  ...
  if (universes.status !== 'active') {
    return (
      <Card className="p-6 h-full">
        <Kicker color="#f59e0b">Universes forming</Kicker>
        <p className="text-[12px] text-token-muted mt-1">
    ...
    <Card className="p-4 h-full" testid="universes-panel">
      <div className="flex items-center justify-between mb-2">
        ...
        <span
          className="inline-flex items-center gap-1 px-2 py-[2px] rounded text-[10px] font-bold tracking-wider"
          style={{
            color: diverged ? '#06b6d4' : 'var(--token-muted)',
    ...
    <Card className="p-4" testid="suppressions-panel">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <Kicker color="#ef4444">Suppressed executions · what AI refused to do</Kicker>
        </div>
        <span
      ...
      >
          ...
          <div>
            ...
            <textarea
              ...
              style={{
                ...
                border: `1px solid ${reason.length > 0 && !reasonValid ? '#ef4444' : 'var(--token-border)'}`,
                outline: 'none',
              }}
          ...
          >
            ...
            <div>
              <div className="text-[12px] font-semibold flex items-center gap-1.5">
                <CircleAlert className="w-3.5 h-3.5" style={{ color: '#f59e0b' }} />
        ...
        >
          ...
          <button
            ...
            style={{
              color: canSubmit ? '#fff' : 'var(--token-muted)',
              background: canSubmit ? '#f59e0b' : 'var(--token-surface)',
              border: '1px solid ' + (canSubmit ? '#f59e0b' : 'var(--token-border)'),
      ...
      {hasAny && (
        <div className="space-y-1.5 max-h-[420px] overflow-auto pr-1" data-testid="memory-items">
          ...
          {memItems.map((d) => {
            ...
            const color =
              oc === 'completed' || oc === 'executed' ? '#10b981' :
              oc === 'failed' || oc === 'rejected' ? '#ef4444' :
              oc === 'suppressed' ? '#f59e0b' :
              'var(--token-muted)';
            return (
              <div
                key={d.log_id}
...
const OverrideMemoryRow = ({ ov }) => {
  ...
  const verdictColor =
    verdict === 'operator_was_correct' ? '#10b981' :
    verdict === 'suppression_was_justified' ? '#ef4444' :
...
const PATTERN_CATEGORY_META = {
  suppression: { icon: Ban,         label: 'SUPPRESSION', color: '#ef4444' },
  qa:          { icon: Shield,      label: 'QA',          color: '#a78bfa' },
  revision:    { icon: RotateCcw,   label: 'REVISION',    color: '#f59e0b' },
  override:    { icon: Hammer,      label: 'OVERRIDE',    color: '#f59e0b' },
...
const ATTRIBUTION_META = {
  ...
  operator_dominant: { color: '#f59e0b', label: 'OPERATOR DOMINANT',
                       sub: 'operators systematically disagreeing' },
  ai_dominant:       { color: '#06b6d4', label: 'AI DOMINANT',
...
const TRACE_PHASE_META = {
  signal_collapse:       { color: '#ef4444', label: 'signal collapse',
                           sub: 'cognition collapse detected' },
  suppressed:            { color: '#f59e0b', label: 'suppressed',
                           sub: 'auto-action held pending review' },
  operator_override:     { color: '#06b6d4', label: 'operator override',
  ...
  outcome_ai_was_right:  { color: '#10b981', label: 'suppression upheld',
                           sub: 'system judgement vindicated' },
  outcome_operator_was_right: { color: '#10b981', label: 'operator vindicated',
